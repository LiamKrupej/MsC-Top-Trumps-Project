...
public class TTModel {
	...
	private int numofdraws; // working
	...
	public void readCards() throws FileNotFoundException {
		...
		File text = new File("src/Model/Empires.txt"); // read from text file
		...
		while (civilizations.hasNextLine()) {
			...
			Deck.add(new Card(NameFirst, NameLast, Size, Duration, Population, Antiquity, CoolFactor)); // adds each
																										// card
																										// incrementally
		} // to deck arraylist
		civilizations.close();
	...
	/*
	 * This method creates the player objects for the game. Always creates 5 players
	 * + 1 communal pile Players can be set to have false activity later, which will
	 * allow number of player selection for the online view
	 */
	...
	// Shuffles the arraylist containing the card objects, this create a randomised
	// deck within the same arraylist
	...
	public void dealCards() {
		...
		int rmndr = Deck.size() % (numOfPlayers + 1); // remainder is used if there is an uneven distribution to deal
														// additional cards
		int deckIndex = 0; // deck index increments every time a card is dealt
		int LDP = 0; // LDP is the last dealt player, in case remainder means additional cards need
						// dealt
		...
		if (rmndr != 0) { // only runs in cases where remainder is not equal to 0, in the case of 5
							// players and
	...
	// get top card returns a read out of the players next card to provide
	// visibility for attribute selection
	...
	// this method gets active players and adds them to an arraylist which is
	// returned
	...
	/*
	 * The compare cards method finds the player with the highest value for the
	 * chosen category, a if two players have the same highest value then this
	 * results in a draw
	 */
	public int compareCards(int a, ArrayList<Player> aP) {
		...
		/*
		 * The if loops check for the highest player by iterating through the active
		 * player arrayList and finding the two highest values this allows us to check
		 * for a draw between the top two player card values currently this outputs each
		 * players top card which should be part of the view attempts to change this
		 * have affected the game logic significantly
		 */
		if (a == 1) {
			for (int j = 0; j < activePlayers.size(); j++) {
				System.out.println(String.format("%s has %s with size %01d", activePlayers.get(j).getPlayerName(),
						activePlayers.get(j).getCardName(), activePlayers.get(j).getGeo()));
				if (j == 0) {
					currentHighest.add(0, activePlayers.get(j)); // adds the first two players to avoid out of bounds
																	// loops
				} else if (j == 1) {
		...
		if (a == 2) {
			for (int j = 0; j < activePlayers.size(); j++) {
				System.out.println(String.format("%s has %s with duration %01d", activePlayers.get(j).getPlayerName(),
						activePlayers.get(j).getCardName(), activePlayers.get(j).getDur()));
		...
		if (a == 3) {
			for (int j = 0; j < activePlayers.size(); j++) {
				System.out.println(String.format("%s has %s with population %01d", activePlayers.get(j).getPlayerName(),
						activePlayers.get(j).getCardName(), activePlayers.get(j).getPop()));
				if (j == 0) {
					currentHighest.add(0, activePlayers.get(j));
				} else if (j == 1) {
					currentHighest.add(1, activePlayers.get(j));
		...
		if (a == 4) {
			for (int j = 0; j < activePlayers.size(); j++) {
				System.out.println(String.format("%s has %s with antiquity %01d", activePlayers.get(j).getPlayerName(),
						activePlayers.get(j).getCardName(), activePlayers.get(j).getAnt()));
				if (j == 0) {
					currentHighest.add(0, activePlayers.get(j));
				} else if (j == 1) {
					currentHighest.add(1, activePlayers.get(j));
		...
		if (a == 5) {
			for (int j = 0; j < activePlayers.size(); j++) {
				System.out
						.println(String.format("%s has %s with cool factor %01d", activePlayers.get(j).getPlayerName(),
								activePlayers.get(j).getCardName(), activePlayers.get(j).getCool()));
				if (j == 0) {
					currentHighest.add(0, activePlayers.get(j));
				} else if (j == 1) {
					currentHighest.add(1, activePlayers.get(j));
			...
			if (currentHighest.get(0).getCool() == currentHighest.get(1).getCool()) {
				...
				System.out.println(String.format("\n %s has the highest card for this round, with a value of %01d",
	...
	/*
	 * add winner cards takes winner, draw and the active players list as its
	 * parameters using these it distributes the cards either to the winning players
	 * hand or in the case of a draw to the communal pile for redistribution in the
	 * next player won round
	 */
	...
	public void addWinnerCards(int w, int d, ArrayList<Player> activePlayers) {
		...
		if (d == 1) {
			...
			for (int j = 0; j < numberActive; j++) { // if not a draw
				playerList.get(w).addToHand(activePlayers.get(j).getTopCard()); // every player, including winners cards
	...
	public int aiChoice(int p, ArrayList<Player> activePlayers) {
		...
		int biggest = Math.max(cardGeo, Math.max(cardDur, Math.max(cardPop, Math.max(cardAnt, cardCool)))); // returns
																											// highest
																											// value
																											// card
		// the next series of if conditionals checks which value is highest and returns,
		// prioritising 1 to 5
		if (cardGeo == biggest) {
	...
	public boolean findWinner(ArrayList<Player> activePlayers) {
		for (int i = 0; i < activePlayers.size(); i++) {
			if (activePlayers.get(i).hand.size() == 40) { // if the players hand holds all 40 cards
				gameWinner = activePlayers.get(i).getPlayerNum(); // they are set as the game winner through their
																	// player number
			}
		}
	...
	// method for finding highest player number still active in the game, used in
	// conditional loop in controller
	...
	// gets the player number from position in activeplayer array, rather than
	// playerlist
